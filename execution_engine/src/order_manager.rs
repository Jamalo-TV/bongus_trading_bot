...
impl OrderManager {
    ...
    async fn check_circuit_breakers(&mut self) -> bool {
        ...
        for (symbol, position) in &self.tracked_positions {
                ...
                (position.spot.as_ref(), position.perp.as_ref())
            {
                ...
                let entry_basis =
                    (perp_pos.entry_price - spot_pos.entry_price) / spot_pos.entry_price;
                let current_basis = (perp_pos.last_mark_price - spot_pos.last_mark_price)
                    / spot_pos.last_mark_price;
                let deviation_bps = ((current_basis - entry_basis).abs()) * 10_000.0;

                if deviation_bps > self.basis_deviation_stop_bps {
                    warn!(
                        "CRITICAL: Basis deviation {:.1}bps exceeds stop {:.0}bps for {}! Emergency flatten.",
    ...
    async fn handle_legging_timeout(&mut self, trigger_client_id: String) {
        ...
        match unfilled_leg {
            Leg::Spot => {
                let _ = self
                    .binance_rest
                    .cancel_order(&unfilled_sym, &unfilled_cid)
                    .await;
            }
            Leg::Futures => {
                let _ = self
                    .binance_rest
                    .cancel_futures_order(&unfilled_sym, &unfilled_cid)
                    .await;
            }
        }
        ...
        if let Ok(body) = market_res {
            ...
            let expected_fill_price = self.paper_market_fill_price(
                &unfilled_sym,
                market,
                unfilled_side,
                fallback_fill_price,
    ...
    async fn try_place_dual_maker(&mut self, symbol: String) {
        ...
        if let Some(active_leg) = chase_snapshot.active_leg() {
            ...
            if let Ok(body) = submission {
                info!("Single-leg unwind order placed: {}", body);
                if let Some(order) = self.internal_orders.get_mut(&client_order_id) {
    ...
    async fn execute_reconciliation_sequence(&mut self) {
        ...
        let open_orders_json = match self.binance_rest.get_open_orders().await {
            ...
            Err(e) => {
                warn!(
                    "Failed to fetch open orders: {}. Will retry reconciliation later.",
                    e
                );
                return;
            }
        ...
        if parsed_orders.is_err() {
            warn!(
                "Failed to parse open orders JSON as array: {}",
                open_orders_json
            );
            ...
            if let Some(client_id) = order.get("clientOrderId").and_then(|v| v.as_str()) {
                ...
                if client_id.starts_with("bngs_") && !self.internal_orders.contains_key(client_id) {
                    ...
                    if let Some(symbol) = order.get("symbol").and_then(|v| v.as_str()) {
                        ...
                        let _ = self
                            .binance_rest
                            .cancel_futures_order(symbol, client_id)
                            .await;
                    }
                }
            }
        ...
        for (client_id, symbol) in dangling {
            ...
            match self
                .binance_rest
                .get_order_by_client_id(LegVenue::UsdtFutures, &symbol, &client_id)
                .await
            {
                Ok(body) => {
...
mod tests {
    ...
    #[tokio::test]
    async fn single_leg_submission_failure_emits_dedicated_reject() {
        let (_event_tx, event_rx) = mpsc::channel(4);
        let (engine_tx, _engine_rx) = mpsc::channel(8);
        let (subscription_tx, _subscription_rx) = mpsc::channel(4);
