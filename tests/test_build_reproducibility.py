...
def test_python_lock_is_exact_and_covers_every_direct_requirement():
    ...
    locked_requirements = [
        Requirement(line) for line in _requirement_lines(PROJECT_ROOT / "requirements.lock")
    ]
...
def test_pinned_toolchain_and_cargo_lock_are_present():
    assert (PROJECT_ROOT / ".python-version").read_text(encoding="utf-8").strip() == "3.11.4"
