...
async def test_runtime_mode_debounce():
    ...
    with patch("bongus.monitoring.telegram_alerter.StateReader") as MockReader, \
        ...
        reader.get_execution_quality_samples.return_value = []
        reader.get_pnl_summary.return_value = {}
        
        start_mono = 10000.0
        
        def _get_risk(*args, **kwargs):
            return {"runtime_mode": "LIVE"} if reader.get_risk.call_count == 1 else {"runtime_mode": "SAFE_MODE", "safe_mode_reason": "test"}

        reader.get_risk.side_effect = _get_risk
