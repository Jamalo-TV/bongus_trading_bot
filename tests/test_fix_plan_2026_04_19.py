...
async def test_runtime_mode_debounce():
    ...
    with patch("bongus.monitoring.telegram_alerter.StateReader") as MockReader, \
        ...
        reader.get_pnl_summary.return_value = {"daily_returns": [0.01, 0.02]}
        reader.get_health_samples.return_value = [{"value": 1.0}]
        reader.get_execution_quality.return_value = [{"value": 0.001}]
        
        start_mono = 10000.0
        
        def risk_side_effect(*args, **kwargs):
            cc = reader.get_risk.call_count
            if cc == 1: return {"runtime_mode": "LIVE"}
            return {"runtime_mode": "SAFE_MODE", "safe_mode_reason": "test"}
        reader.get_risk.side_effect = risk_side_effect
