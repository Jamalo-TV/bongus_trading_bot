...
async def test_runtime_mode_debounce():
    ...
    with patch("bongus.monitoring.telegram_alerter.StateReader") as MockReader, \
        ...
        def side_effect():
            yield {"runtime_mode": "LIVE"} # priming
            yield {"runtime_mode": "SAFE_MODE", "safe_mode_reason": "test"} # loop 1
            yield {"runtime_mode": "SAFE_MODE", "safe_mode_reason": "test"} # loop 2
            while True:
                yield {"runtime_mode": "SAFE_MODE", "safe_mode_reason": "test"}

        gen = side_effect()
        reader.get_risk.side_effect = lambda: next(gen)
